...
async function getLiveGameStats() {
    ...
    const inProgressGames = schedule.data.dates[0].games.filter(g => g.status.statusCode === 'I');
    ...
    for(let i=0; i<inProgressGameLinks.length; i++) {
        ...
        const currentInfo = {gameId, matchup, playEvents}
...
// setInterval(getLiveGameStats, .25 * 60 * 1000);
