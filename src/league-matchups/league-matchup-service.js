...
const testSomeShit = async (userId, leagueId) => {

    const leagueSettings = await yahooApi.yfbb.getLeagueSettings(userId, leagueId);
    const statCategoryInfo = leagueSettings.settings.stat_categories.stats.stat;
    const teamId = 6;
    const currentWeek = await yahooApi.yfbb.getCurrentWeek(userId, leagueId);

    const mySchedule = await yahooApi.yfbb.getTeamMatchups(userId, leagueId, teamId)
    ...
    const leagueSeasonStats = await yahooApi.yfbb.getLeagueSeasonStats(userId, leagueId);

    const matchups = await yahooApi.yfbb.getMyScoreboard(userId, leagueId, currentWeek)
    ...
    for (let i = 0; i < leagueSeasonStats.standings.teams.team.length; i++) {
        ...
        if (onlyGetCurrentOpponentWeeklyStats && (myMatchupTeamIds.includes(parseInt(team.team_id)))) {
            for (let j = currentWeek - 1; j > currentWeek - 4; j--) {
                const weekStats = await yahooApi.yfbb.getWeeklyStats(userId, leagueId, team.team_id, j);
